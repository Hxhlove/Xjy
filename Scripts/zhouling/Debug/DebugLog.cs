...
static class DebugLog
{
    ...
    private static void WriteLogHead()
    {
        try
        {
            ...
            //运行时间
            writer.WriteLine("开始时间:{0}", DateTime.Now.ToString());
            //设备名称，及类型
            ...
            //渲染设备及版本，显存信息
            writer.WriteLine("渲染设备:{0},版本号:{1},显存信息:{2}", SystemInfo.graphicsDeviceName, SystemInfo.graphicsDeviceVersion, SystemInfo.graphicsMemorySize);
            //CPU设备类型及内核数，频率
            writer.WriteLine("CPU设备:{0},内核数:{1},频率:{2}", SystemInfo.processorType, SystemInfo.processorCount, SystemInfo.processorFrequency);
            //设备唯一序号
    ...
    internal static void WriteLog(string logstr)
    {
        ...
        {
            Debug.LogError(string.Format("加入日志错误:\r\n{0}\r\n{1}\r\n{2}", logstr, err.StackTrace, err.Message));
        }
        finally
        {
            if (writer != null)
